...
def inject_default_parser(parser=None):
    ...
    parser.add_argument('--snapshot', default='./weights/coff-3dmatch.pth.tar',
...
class BaseTester(abc.ABC):
    def __init__(self, cfg, parser=None, cudnn_deterministic=True):
        ...
        # logger
        log_file = osp.join(cfg.log_dir, f"test-{self.args.benchmark}-{time.strftime('%Y%m%d-%H%M%S')}.log")
    ...
    def load_snapshot(self, snapshot):
        ...
        state_dict_all = torch.load(snapshot, weights_only=True, map_location=torch.device('cpu'))
        assert 'model' in state_dict_all, 'No model can be loaded.'

        self.model.load_state_dict(state_dict_all['model'], strict=True)
