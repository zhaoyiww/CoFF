...
class EpochBasedTrainer(BaseTrainer):
    ...
    # save img model
    def _load_pretrain_img_LCD(self, checkpoint_file):
        import os
        if osp.isfile(checkpoint_file):
            state = torch.load(checkpoint_file, weights_only=True)
            self.model_img.load_state_dict(state['patchnet'])
            # print(f'Successfully load pretrained model from {checkpoint_file}!\n')
        else:
            raise ValueError(f"=> no checkpoint found at '{checkpoint_file}'")
    ...
    def run(self):
        ...
        if self.args.resume:
            ...
            # load model_img
            self.load_snapshot_img(osp.join(self.snapshot_dir, 'img_snapshot.pth.tar'))
        elif self.args.snapshot is not None:
            self.load_snapshot(self.args.snapshot)
            # load model_img
            self.load_snapshot_img(osp.join(self.snapshot_dir, 'img_snapshot.pth.tar'))
        else:
            # load LCD img pretraine model
            pretrain_path = 'src/backbone_2d_patchnet/LCD-D256/model.pth'
            self._load_pretrain_img_LCD(pretrain_path)
        self.set_train_mode()
