...
from utils.common import dir_exist, transfrom2_rot_tran
from utils.summary_board import SummaryBoard
from utils.metrics import apply_transform, get_nearest_neighbor
from utils.metrics import compute_overlap, compute_registration_error
...
def write_log_file(file_name, test_pairs):
    dir_exist(osp.dirname(file_name))
    lines = []
...
def get_gt_logs(gt_root, num_fragments):
    gt_logs = read_log_file(osp.join(gt_root, 'gt.log'))
...
def compute_transform_error(transform, covariance, estimated_transform):
    ...
    R, t = transfrom2_rot_tran(relative_transform)
    q = nq.mat2quat(R)
