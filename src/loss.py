...
from src.backbone_3d.modules.loss import WeightedCircleLoss
from src.backbone_3d.modules.ops.transformation import apply_transform
from src.backbone_3d.modules.registration.metrics import isotropic_transform_error
...
class ImgMatchingLoss(nn.Module):
    def __init__(self, cfg):
        super(ImgMatchingLoss, self).__init__()
        self.weighted_circle_loss = WeightedCircleLoss(
            cfg.coarse_loss.positive_margin,
            cfg.coarse_loss.negative_margin,
            cfg.coarse_loss.positive_optimal,
    ...
    def forward(self, output_dict):
        ...
        feat_dists = torch.sqrt(pairwise_distance(img_patch_ref_feats, img_patch_src_feats, normalized=True))

        overlaps = torch.zeros_like(feat_dists)
        overlaps[sel_gt_ref_node_corr_indices, sel_gt_src_node_corr_indices] = selected_overlaps
        pos_masks = torch.gt(overlaps, self.positive_overlap)
        neg_masks = torch.eq(overlaps, 0)
        pos_scales = torch.sqrt(overlaps * pos_masks.float())
...
class OverallLoss(nn.Module):
    def __init__(self, cfg):
        ...
        self.img_matching_loss = ImgMatchingLoss(cfg)
        self.weight_img_matching_loss = cfg.loss.weight_img_matching_loss
...
# source: predator
def get_recall(coords_dist,feats_dist):
    """
    ...
    n_pred_pos = (sel_dist < pos_radius).float().sum()
...
class Evaluator(nn.Module):
    ...
    def forward(self, output_dict, data_dict):
        ...
        return {
            ...
            'RMSE': rmse,
            'RR': recall,
            # 'RR_pre_c': recall_predator_c,
            # 'RR_pre_f': recall_predator_f
        }
