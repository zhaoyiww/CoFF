...
# ScanNetpp_test or ScanNetpp_test_planar
benchmark = "ScanNetpp_test"
dataset = "ScanNetpp"
...
for scene in scenes:
    ...
    est_path = f"../outputs/ScanNetpp/registration/{benchmark}/{scene}/est.log"

    pairs, pose_gt = read_log(gt_path)
    sorted_indices = np.lexsort((pairs[:, 0], pairs[:, 1]))
    ...
    for i in tqdm(range(pose_est.shape[0])):
        # point cloud path
        tgt_pts_path = os.path.join(f"../data/ScanNetpp/data/test/{scene}", f"cloud_bin_{pairs_sorted[i, 0]}.npz")
