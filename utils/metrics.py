# ---------------------------------------------------------------------------- #
# Metrics for 3D point cloud registration and evaluation
# Modified from GeoTrans and other sources
# ---------------------------------------------------------------------------- #
...
def compute_registration_error(gt_transform: np.ndarray, est_transform: np.ndarray):
    ...
    Returns:
        ...
        rte (float): relative translation error.
    """
    gt_rotation, gt_translation = transfrom2_rot_tran(gt_transform)
    est_rotation, est_translation = transfrom2_rot_tran(est_transform)
...
def compute_inlier_ratio_2(ref_corr_points, src_corr_points, transform, positive_radius=0.1):
    r"""Computing the inlier ratio between a set of correspondences."""
    src_corr_points = apply_transform(src_corr_points, transform)
    residuals = np.sqrt(((ref_corr_points - src_corr_points) ** 2).sum(1))
    inlier_ratio = np.mean(residuals < positive_radius)
    mask = residuals < positive_radius
    return inlier_ratio, mask
...
def evaluate_correspondences_2(ref_points, src_points, transform, positive_radius=0.1):
    overlap = compute_overlap(ref_points, src_points, transform, positive_radius=positive_radius)
    inlier_ratio, mask = compute_inlier_ratio_2(ref_points, src_points, transform, positive_radius=positive_radius)
    residual = compute_correspondence_residual(ref_points, src_points, transform)
...
def evaluate_sparse_correspondences_2(ref_points, src_points, ref_corr_indices, src_corr_indices, gt_corr_indices):
    ref_gt_corr_indices = gt_corr_indices[:, 0]
    src_gt_corr_indices = gt_corr_indices[:, 1]
    ...
    # mask_mutual = np.where(pos_corr_mat > 0)
    neg_corr = pred_corr_mat - pos_corr_mat
