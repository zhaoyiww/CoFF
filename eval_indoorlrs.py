...
from src.backbone_3d.modules.registration import weighted_procrustes
from utils.summary_board import SummaryBoard
from utils.o3d_tools import ransac_registration
from utils.metrics import (
    evaluate_sparse_correspondences,
    evaluate_correspondences,
    compute_registration_error,
)
from src.dataloaders.datasets.threedmatch_utils import (
    get_num_fragments,
    get_scene_abbr,
    get_gt_logs,
...
def eval_one_epoch(args, cfg, logger):
    ...
    for scene_root in scene_roots:
        ...
        gt_indices, gt_logs = get_gt_logs(gt_root, num_fragments)
        ...
        for file_name in file_names:
            ...
            if args.method == 'lgr':
                ...
                estimated_transform = ransac_registration(
                    src_corr_points,
                    ref_corr_points,
...
def main():
    ...
    parser.add_argument('--benchmark', default='IndoorLRS_planar',
                        choices=['IndoorLRS', 'IndoorLRS_planar'], help='test benchmark')
    parser.add_argument('--method', default='lgr', choices=['lgr', 'ransac', 'svd'], help='registration method')
    ...
    log_file = osp.join(cfg.log_dir, f"eval-{args.benchmark}-{time.strftime('%Y%m%d-%H%M%S')}.log")
